...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    """
    if (temperature * neutrons_emitted < 500000) and (temperature < 800 and neutrons_emitted > 500): #500.000 wrong - 500000 right
        return True
    return False
...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    totals = (voltage * current / theoretical_max_power) * 100
    if totals >= 80:
        return 'green'
    if totals >= 60:    #elif Total <80 and >= 60:
        return 'orange'
    if totals >= 30:    #elif Total <60 and >= 30:
        return 'red'
    return 'black'        #else:                #elif Total < 30:
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    keamanan = int(temperature * neutrons_produced_per_second)
    if keamanan < (int(threshold * 0.9)):
        return 'LOW'
    if int(threshold * 0.9) <= keamanan < int(threshold * 1.1):    #elif keamanan >= (int(threshold * 0.9)) and keamanan < (int(threshold * 1.1)):
        return 'NORMAL'
    return 'DANGER'                        #else:
